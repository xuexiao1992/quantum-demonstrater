...
SPC_READY = 20
SPC_M2CMD = 100
M2CMD_CARD_RESET = 0x00000001                   #Performs a hard and software reset of the card
M2CMD_CARD_WRITESETUP = 0x00000002              #Writes the current setup to the card without starting the hardware
M2CMD_CARD_START = 0x00000004                   #Starts the card with all selected settings
M2CMD_CARD_ENABLETRIGGER = 0x00000008           #the trigger detection is enabled
M2CMD_CARD_FORCETRIGGER = 0x00000010            #this command forces a trigger even if none has been detected so far
M2CMD_CARD_DISABLETRIGGER = 0x00000020          #the trigger detection is disabled
M2CMD_CARD_STOP = 0x00000040                    #stops the current run of the card
M2CMD_CARD_FLUSHFIFO = 0x00000080
M2CMD_CARD_INVALIDATEDATA = 0x00000100
M2CMD_ALL_STOP = 0x00440060
M2CMD_CARD_WAITPREFULL = 0x00001000             #Acquisition modes only: the command waits until the pretrigger area has once been filled with data
M2CMD_CARD_WAITTRIGGER = 0x00002000             #Waits until the first trigger event has been detected by the card
M2CMD_CARD_WAITREADY = 0x00004000               #Waits until the card has completed the current run
M2CMD_DATA_STARTDMA = 0x00010000                #Starts the DMA transfer for an already defined buffer
M2CMD_DATA_WAITDMA = 0x00020000                 #Waits until the data transfer has ended or until at least the amount of bytes defined by notify size are available
M2CMD_DATA_STOPDMA = 0x00040000                 #Stops a running DMA transfer. Data is invalid afterwards
M2CMD_DATA_POLL = 0x00080000
M2CMD_EXTRA_STARTDMA = 0x00100000               #See above, but for the timestamp and ABA FIFO
M2CMD_EXTRA_WAITDMA = 0x00200000                #See above, but for the timestamp and ABA FIFO
M2CMD_EXTRA_STOPDMA = 0x00400000                #See above, but for the timestamp and ABA FIFO
M2CMD_EXTRA_POLL = 0x00800000                   #See above, but for the timestamp and ABA FIFO
SPC_M2STATUS = 110                      
M2STAT_NONE = 0x00000000
M2STAT_CARD_PRETRIGGER = 0x00000001             #Acquisition modes only: the pretrigger area has been filled
M2STAT_CARD_TRIGGER = 0x00000002                #The first trigger has been detected
M2STAT_CARD_READY = 0x00000004                  #The card has finished its run and is ready
M2STAT_CARD_SEGMENT_PRETRG = 0x00000008
M2STAT_DATA_BLOCKREADY = 0x00000100             #The next data block as defined in the notify size is available (can also be more data)
M2STAT_DATA_END = 0x00000200                    #The data transfer has completed. This will occur if the notify size is set to 0.
M2STAT_DATA_OVERRUN = 0x00000400                #The data transfer had an overrun (acquisition) or underrun (replay) while doing FIFO transfer
M2STAT_DATA_ERROR = 0x00000800                  #An internal error occured while doing data transfer
