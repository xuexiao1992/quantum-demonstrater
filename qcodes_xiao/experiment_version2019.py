...
from manipulation_library import Ramsey, CRot
...
#                    break
    ...
    def add_Y_parameter(self, measurement, parameter, sweep_array, **kw):
        ...
        self.pre_stored = kw.pop('pre_stored', False)
...
#            self.update_segment_into_sequence_and_elts(segment, repetition, idx_j = idx_j, idx_i = idx_i)
    ...
    def add_compensation(self, idx_j = 0, idx_i = 0, seq_num = 0):
        ...
        seq_cfg = self.sequencer[seq_num].sequence_cfg
        seq_cfg_type = self.sequencer[seq_num].sequence_cfg_type
        
        for segment_num in range(len(seq_cfg_type)):
            if seq_cfg_type[segment_num].startswith('manip'):
                
                seg_name = seq_cfg_type[segment_num]
                step = self.segment[seg_name]['step1']
                
                if type(step) is list:
                    dimension_j = len(step)
                    dimension_i = len(step[0])
                    j = idx_j if idx_j < dimension_j else dimension_j-1
                    i = idx_i if idx_i < dimension_i else dimension_i-1
                    print('J and I :', j, i)
                    print('segment:', seg_name)
                    element = step[j][i]
                
#                elif type(step) is CRot:
                elif issubclass(type(step), Element):
                    element = step
                    
                else:
                    raise KeyError('cannot distinguish step_%d_%d in %s'%(idx_j, idx_i, seg_name))
                tvals, wfs = element.ideal_waveforms()
                
                amplitude[0] += np.sum(wfs[self.channel_VP[0]])
                amplitude[1] += np.sum(wfs[self.channel_VP[1]])
#                print(seg_name, 'done', 'amp0:', amplitude[0])
            
#            elif seq_cfg_type[segment_num] == 'manip2':
#                pass
            else:
                for step in seq_cfg[segment_num]:
                    amplitude[0] += 1e9 * seq_cfg[segment_num][step]['time'] * seq_cfg[segment_num][step]['voltage_1']
                    amplitude[1] += 1e9 * seq_cfg[segment_num][step]['time'] * seq_cfg[segment_num][step]['voltage_2']
#                    print(seq_cfg_type[segment_num], 'done', 'amp0:', amplitude[0])
        ...
        if idx_j == 0 and idx_i == 0 and seq_num == 0:
#        if 1:
        ...
        if idx_i == 0 and seq_num == 0:
            
            self.elts.append(self.zerobias_element)

        self.sequencer[seq_num].elts.append(self.zerobias_element)
        
        
        self.sequence.append(name = 'zerobias_%d_%d'%(seq_num, idx_i), 
                             wfname = 'zerobias_%d_%d'%(0, 0), 
...
#                self.Loop = Loop(sweep_values = Sweep_Count).each(LOOP)
    ...
    def calibrate_by_Ramsey(self, qubit = 0):
        ...
        frequency = VSG.frequency()+frequency_shift #in XX's version the frequency shift was multiplied 0.3
