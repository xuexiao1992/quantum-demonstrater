...
class Ramsey2(Circuit):
    ...
    def __init__(self, name, pulsar, **kw):
        ...
        self.add_X(name='X1_Q1', qubit = self.qubit,
                   amplitude = self.amplitude, length = self.length, frequency_shift = self.frequency_shift)
        
        self.add_Z(name='Z1_Q1', qubit = self.qubit, degree = self.phase_2)

        self.add_X(name='X2_Q1', refgate = 'X1_Q1', qubit = self.qubit, waiting_time = self.waiting_time,
                   amplitude = self.amplitude, length = self.length, frequency_shift = self.frequency_shift)
